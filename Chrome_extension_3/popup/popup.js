function extractProductNumberFromURL(url) {
  ...
  function showContent(data){
    ...
    for (let i in data) {
        ...
        if (j === 'ID') {
        ...
        else if (j === '상품명' || j === '특징' || j === 'ESG_Score') {
            ...
            mainCell.textContent = content;
            }
            
        }
        else if (j === 'E_Grade' || j === 'S_Grade' || j === 'G_Grade') {
            const hoverCell = hoverRow.insertCell(); // 여분의 행 셀
            const content = j + ' : ' + data[i][j];
            hoverCell.textContent = content; // 여분의 행에 내용을 삽입
            hoverRow.style.display = 'none'; // 여분의 행은 기본적으로 숨김 처리
        }
        }
        hoverRow.style.backgroundColor = 'lightgreen'
        ...
        // 호버를 벗어날 때 여분의 행 숨김
        mainRow.addEventListener('mouseout', function () {
